...
async function findNsuidsPhase1(gameUrl, emit) {
  ...
  // ── Priority 1: titledb-us local word match → US NSUID + xref seeds ──────────
  {
    ...
    if (match.length) {
      ...
      const entry = usEntries.find(e => e.nsuid === usNsuid);
      if (entry) {
        // Set gameName early so we can skip network calls if local data is complete
        if (!gameName) gameName = entry.nameEn || entry.name;
  ...
  // If local data already gave us US + JP + HK, skip all network calls in Phase 1.
  // US covers CA/BR/MX (same Americas NSUID), AU and SG are resolved in Phase 2.
  const localComplete = !!(usNsuid && jpNsuids.length && hkNsuids.length);

  // originalSlug: for nintendo.com inputs, the full slug before stripping
  const nintendoMatch = gameUrl.match(/nintendo\.com\/[a-z]{2}\/store\/products\/([^/?#]+)/i);
  const originalSlug = nintendoMatch ? nintendoMatch[1] : null;

  let algoliaKey = null;
  if (localComplete && !originalSlug && !gameId) {
    emit('Local data complete — skipping network discovery');
  } else {
    // EU catalog + JP store search + Algolia key in parallel
    [,, algoliaKey] = await Promise.all([
      (async () => {
        try {
          const res = await axios.get(`https://searching.nintendo-europe.com/en/select?q=${q}&fq=type%3AGAME&rows=10&wt=json&fl=title,nsuid_txt`, { timeout: 6000 });
          ...
          if (scored[0]?.d.title && !gameName) gameName = scored[0].d.title;
      ...
      // store-jp.nintendo.com search — only run if JP not already seeded
      jpNsuids.length ? Promise.resolve() : (async () => {
      ...
      // Skip Algolia key scan if we already have the US NSUID
      usNsuid ? Promise.resolve(null) : getAlgoliaKey(emit),
    ]);
        ...
        for (const indexName of ['store_game_en_us_release_date', 'store_game_en_us', 'noa_aem_game_en_us']) {
          try {
            const res = await axios.post('https://u3b6gr4ua3-dsn.algolia.net/1/indexes/*/queries',
              ...
              { headers: { 'X-Algolia-Application-Id': ALGOLIA_APP_ID, 'X-Algolia-API-Key': algoliaKey, 'Content-Type': 'application/json' }, timeout: 5000 }
            );
...
// switchbrew region codes → our region codes (CHN = HK eshop)
const SWITCHBREW_REGION_MAP = { JPN: 'JP', EUR: 'EU', USA: 'US', CHN: 'HK', KOR: 'KR', AUS: 'AU' };
...
// Find titleId from switchbrew by game name + target region (e.g. 'JP', 'HK').
// Returns null if no region-specific entry found (no fallback to unrelated region).
function getTitleIdFromSwitchbrew(gameName, region, emit) {
  ...
  if (!swRegion) return null;
  const result = matches.find(e => e.regions.includes(swRegion));
...
async function findNsuidsPhase2(gameUrl, { seen, gameName, euNsuids, jpNsuids, hkNsuids, usNsuid }, emit) {
  ...
  async function findHK() {
    ...
    // 1b. ec.nintendo.com redirect via titleId — only when CHN in switchbrew (game is on HK eshop)
    ...
    if (newNsuids.length === beforeCount) {
      const tdIds = findNsuidsViaTitledb('HK', gameName, emit);
      tdIds.forEach(addNew);
    }

    // 3. Gap probe off EU/US anchor
    if (newNsuids.length === beforeCount) {
  ...
  async function findJP(hkBase = []) {
    ...
    if (newNsuids.length === beforeCount) {
      // Prefer JP-specific titleId from switchbrew (handles per-region splits like Cyberpunk)
      const titleId = getTitleIdFromSwitchbrew(gameName, 'JP', emit) || (usNsuid ? getTitleIdForNsuid(usNsuid, emit) : null);
      if (titleId) {
    ...
    const tdIds = findNsuidsViaTitledb('JP', gameName, emit);
    tdIds.forEach(addNew);
    ...
    try {
      ...
      const ids = [...new Set((String(res.data).match(/D(700[0-9]\d{10})/g) || []).map(m => m.slice(1)))];
      emit(`JP store search: ${ids.length} nsuid(s)${ids.length ? ` [${ids.join(',')}]` : ''}`);
      ids.forEach(addNew);
      if (ids.length) return;
    } catch (e) { emit(`JP store search: ${e.message.slice(0, 60)}`); }

    // 4. Gap probe off US NSUID
    if (usNsuid) {
      const JP_GAP = 50n;
    ...
    // 5. Gap probe off HK NSUIDs — JP and HK NSUIDs are often 1-2 apart
    if (hkBase.length) {
      const JP_GAP = 10n;
      const probeIds = [...new Set(hkBase.flatMap(b => {
        const bn = BigInt(b);
...
async function buildResults(gameUrl, emit, onPartial) {
  ...
  // Skip second price API call if Phase 2 added no new NSUIDs (saves ~2s)
  const p2Prices = finalNsuids.length > phase1.nsuids.length
    ? await getNintendoPrices(finalNsuids, emit)
    : p1Prices;
  ...
  bot.on('message', async (msg) => {
    ...
    if (match) {
      ...
      try {
        ...
        await bot.sendMessage(chatId,
          `🧩 *${escGc(solverCur)} ${escGc(solverAmount.toFixed(2))}* — ${escGc(results.length + ' combination' + (results.length > 1 ? 's' : '') + ':')}`,
          { parse_mode: 'MarkdownV2' });
        for (const combo of results) {
          const sum = combo.reduce((a, g) => a + g.price, 0);
          const lines = [escGc(combo.length + ' game' + (combo.length > 1 ? 's' : '') + ' (' + fmt(sum) + ')')];
